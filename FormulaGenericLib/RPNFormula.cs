...
{
	public class RPNFormula<T, U>
		where T : struct,MathProvider<U>
		where U : struct
	{
		/// <summary>
				...
				switch (mode)
				{
					...
					case NodeType.Constant:
						...
						else if (IsOperator(curChar) || IsRightBrackets(curChar))
						{
							...
							if (!defT.TryParse(source.Substring(startCharIndex, charIndex - startCharIndex), out value))
							{
								return new CompileError
								{
		...
		public int SetVariable(string name, U value)
		{
			int numFound = 0;
			...
			for (int i = 0; i < numNodes; i++)
			{
				...
				else
				{
					...
					else
					{
						...
						{
							case NodeType.Add:
								result.Value = defT.Add(op2.Value, op1.Value);
								break;
							case NodeType.Subtract:
								result.Value = defT.Subtract(op2.Value, op1.Value);
								break;
							case NodeType.Multiply:
								result.Value = defT.Multiply(op2.Value, op1.Value);
								break;
							case NodeType.Divide:
								result.Value = defT.Divide(op2.Value, op1.Value);
								break;
							case NodeType.Modulus:
								result.Value = defT.Modulus(op2.Value, op1.Value);
								break;
						}
